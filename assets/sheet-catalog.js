window.SheetCatalog=function(options){
  ...
  dialog.innerHTML='<div class="catalog-head"><div><span class="eyebrow">Códice de Valédria</span><h2 id="catalog-title"></h2></div><button type="button" class="catalog-close" aria-label="Fechar catálogo" title="Fechar"><svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" aria-hidden="true"><path d="m6 6 12 12M18 6 6 18"/></svg></button></div><div class="catalog-tools"><label>Buscar<input type="search" id="catalog-search" placeholder="Nome ou descrição"></label><label><span id="catalog-filter-label">Categoria</span><select id="catalog-filter"></select></label></div><p id="catalog-count" role="status" aria-live="polite"></p><div id="catalog-grid" class="catalog-grid"></div><div class="catalog-foot"><span>As escolhas aparecem na sua ficha.</span><button type="button" class="btn btn-primary catalog-done">Concluir</button></div>';

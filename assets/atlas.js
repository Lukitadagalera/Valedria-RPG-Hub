...
(function(){
  ...
  function size(){var old=base||Math.max(1000,viewport.clientWidth);var cx=(viewport.scrollLeft+viewport.clientWidth/2)/(old*zoom);var cy=(viewport.scrollTop+viewport.clientHeight/2)/(old*zoom*ratio);base=Math.max(1000,viewport.clientWidth);wrap.style.width=base*zoom+'px';viewport.scrollLeft=cx*base*zoom-viewport.clientWidth/2;viewport.scrollTop=cy*base*zoom*ratio-viewport.clientHeight/2;}
  function setZoom(next){var w=wrap.clientWidth;var cx=(viewport.scrollLeft+viewport.clientWidth/2)/w,cy=(viewport.scrollTop+viewport.clientHeight/2)/(w*ratio);zoom=Math.max(1,Math.min(3,next));wrap.style.width=base*zoom+'px';viewport.scrollLeft=cx*base*zoom-viewport.clientWidth/2;viewport.scrollTop=cy*base*zoom*ratio-viewport.clientHeight/2;document.getElementById('atlas-in').disabled=zoom===3;document.getElementById('atlas-out').disabled=zoom===1;}

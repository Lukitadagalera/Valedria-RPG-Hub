...
const [action,email,date]=process.argv.slice(2);
...
try{
 ...
 if(action==='grant'){
  ...
  db.prepare('INSERT INTO entitlements VALUES(?,?,?) ON CONFLICT(user_id) DO UPDATE SET expires=excluded.expires,source=excluded.source').run(user.id,expires,'operator');
